...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define :chefs do |server|
    ...
    server.vm.provision "shell", inline: "echo 'Downloading chef server....';curl -L -s -C - -o /chef/server/chef-server-core_12.1.0-1_amd64.deb https://web-dl.packagecloud.io/chef/stable/packages/ubuntu/trusty/chef-server-core_12.1.0-1_amd64.deb;exit 0"
    server.vm.provision "shell", inline: "DEBIAN_FRONTEND=noninteractive dpkg -i /chef/server/chef-server-core_12.1.0-1_amd64.deb"
  ...
  config.vm.define :chefd do |dev|
    ...
    dev.vm.provision "shell", inline: "echo 'Downloading chef dk....';curl -s -L -C - -o /chef/dev/chefdk_0.6.2-1_amd64.deb https://opscode-omnibus-packages.s3.amazonaws.com/ubuntu/12.04/x86_64/chefdk_0.6.2-1_amd64.deb;exit 0"
    dev.vm.provision "shell", inline: "DEBIAN_FRONTEND=noninteractive dpkg -i /chef/dev/chefdk_0.6.2-1_amd64.deb"
